    ...
    async def generate_response(self, text: str, chat_context: ChatContext, media_list: List[dict] = None, sender_info: dict = None) -> Tuple[str, List[FunctionCall]]:
        ...
        try:
            ...
            tool_constraints = (
                ...
                f"8. If the user asks you to find, search for, or identify the source of an image, you MUST ALWAYS use the `download_media_to_disk` tool followed by the `reverse_image_search` tool.\n"
                f"9. Keep your text responses clean and natural, containing only what you would actually type in a chat. Avoid excessive Markdown formatting unless specifically asked for a structured list.\n"
            )
